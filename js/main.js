...
const formActionBtn = document.querySelectorAll('.actionBtn')
const formBtn = document.querySelector('.submitBtn')
const responseMsgField = document.querySelector('.responseMsg')

window.addEventListener('load', (e) => {
    let cnpjField = mainForm.querySelector('#cnpj')
    let cpfField = mainForm.querySelector('#cpf')
    let telefoneField = mainForm.querySelector('#telefone')
    let cepField = mainForm.querySelector('#cep')
    let valueField = mainForm.querySelector('#valor')
    let fields = [cpfField, cnpjField, telefoneField, cepField, valueField]
    let decodedCookie = decodeURIComponent(document.cookie);
    
    fields.forEach((field) => {
        if(field !== undefined && field !== null){
            if(field.value !== ""){
                if(field.id === "telefone" && field.value.length === 10) {
                    field.value = formatText(field.value, "telefone_fixo")
                } else {
                    field.value = formatText(field.value, field.id)
                }
            }
        }  
    })
...
mainForm.addEventListener('click', (e) => {
    ...
    if(element.classList.contains('actionBtn')){
        const formPages = document.querySelectorAll('.page')
        const nextPageBtn = document.querySelector('#nextPage')
        const returnPageBtn = document.querySelector('#returnPage')
    
        for(let i=0; i < formPages.length; i++){
            nextPageBtn.classList.remove('hideBtn')
            returnPageBtn.classList.remove('hideBtn')
            formBtn !== null ? formBtn.classList.add('hideBtn') : null
    
            if(!formPages[0].classList.contains('hidePage') && element.id === 'returnPage'){
                return
            }
            if(!formPages[i].classList.contains('hidePage')){
                let page;
                if(element.id === 'nextPage'){
                    returnPageBtn.classList.remove('hideBtn')
                    page = i+1
                    if(page === formPages.length-1){
                        nextPageBtn.classList.add('hideBtn')
                        formBtn !== null ? formBtn.classList.remove('hideBtn') : null
                    }
                } else if (element.id === 'returnPage'){
                    page = i-1
                    page === 0 ? returnPageBtn.classList.add('hideBtn') : null
                }
    
                if(page < formPages.length){
                    formPages[i].classList.add('hidePage')
                    formPages[page].classList.remove('hidePage')
                }
                return
            }
        }
    } else if(element.classList.contains('controlPass')){
        const elementChildren = element.parentElement.children
        const passInput = elementChildren[0]
        const showPass = elementChildren[1]
        const hidePass = elementChildren[2]
...
mainForm.addEventListener('change', (e) => {
    ...
    switch (element.id) {
        ...
        case 'documento':
            ...
            element.value !== "" ? addNewDocBtn.style.display = 'block' : addNewDocBtn.style.display = 'none'
            break;
        default:
            break;
    }
...
mainForm.addEventListener('input', async (e) => {
    const element = e.target
    let minLength;
    let maxLength;
    let file;
    ...
    switch (element.id) {
        ...
        case 'relatorio_up':
            ...
            if(file.type !== 'application/pdf'){
                responseMsgField.innerHTML = 'Você deve enviar um arquivo em formato PDF!'
                element.files[0] = ''
                element.labels[0].innerHTML = 'Enviar Arquivo'
                return
            }
            element.labels[0].innerHTML = file.name
            break;
        ...
        case 'nova_senha':
            ...
            if(element.value.length < minLength){
                return responseMsgField.innerHTML = `O campo '${element.labels[0].innerHTML}' precisa possuir no mínimo ${minLength} caracteres!`
            } else if(element.value.length > maxLength){
                return responseMsgField.innerHTML = `O campo '${element.labels[0].innerHTML}' precisa possuir no máximo ${maxLength} caracteres!`
            } else {
...
formBtn.addEventListener('click', (e) => {
    ...
    if(!e.target.parentElement.classList.contains('form')){
    ...
    for(const input of inputs){
        if(!nullInputs.includes(input.id) && (input.value === "" || input.value === null || input.value === undefined)){
            return responseMsgField.innerHTML = `O campo '${input.labels[0].innerHTML}' precisa ser preenchido!`
        }
        ...
        switch (input.id) {
            case 'cnpj':
                ...
                if(formatedText.length !== maxLength){
                    return responseMsgField.innerHTML = `O campo '${input.labels[0].innerHTML}' precisa possuir ${maxLength} caracteres!`
                }
                break;
            case 'cpf':
                formatedText = formatText(input.value, input.id, 'remove')
            ...
            case 'nova_senha':
                ...
                break;
            case 'valor':
                formatedText = formatText(String(input.value), input.id, 'remove')
                formatedElements.push([input, formatedText])
            default:
                break;
        }
...
function blockText(length, isNum, element){
    if(element.value.length < length && isNaN(element.value[element.value.length-1]) === isNum){
...
function finishAction(msg, nextPage=null, seconds=5) {
    if(seconds !== 0){
        responseMsgField.innerHTML = msg + `${seconds}s`;
        if(nextPage !== null){
            seconds--;
...
async function getAddressData(cep){
    ...
    if(!addressData_o.error){
        ...
        const numField = mainForm.querySelector('#num')
        const fields = [logradouroField, bairroField, cidadeField, estadoField]
        ...
        switch(element.id){
            case 'addNewDoc':
                if(allDocs.length < 5){
                    ...
                    newDoc.innerHTML = docElement.innerHTML
        
                    const newDocBtn = newDoc.querySelector('.controlBtns')
