                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . '?' . $_SERVER['QUERY_STRING'])?>" method="post" class="form" enctype="multipart/form-data">
                    ...
                    ?>
                            <div class="page">
                                ...
                                <input type="text" name="cnpj" id="cnpj" value="<?php echo $o_cnpj ?>">
                            ...
                            <div class="page hidePage">
                                ...
                                <input type="text" name="login" id="login" value="<?php echo $o_username ?>">
                    ...
                    <?php        
                            ...
                            $o_dados_escola = mysqli_fetch_assoc(mysqli_query($mysql, "SELECT * FROM escolas WHERE id = ". $row['escola_id']));
                            $o_escola = $o_dados_escola['nome'];
                            $o_escola_id = $o_dados_escola['id'];
                    ...
                    ?>
                            <div class="page">
                                ...
                                <input type="text" name="cpf" id="cpf" value="<?php echo $o_cpf ?>">
                                <label for="telefone">Telefone</label>
                                <input type="text" name="telefone" id="telefone" value="<?php echo $o_telefone ?>">
                                <label for="email">E-mail</label>
                                <input type="email" name="email" id="email" value="<?php echo $o_email ?>">
                                ...
                                <select name="escola" id="escola">
                                    <option value="<?php echo $o_escola_id; ?>" selected hidden><?php echo $o_escola ?></option>
                                    <?php 
            ...
            if($type === 'emp'){
                ...
                if(move_uploaded_file($relatorio["tmp_name"], $local_arquivo)){
                    ...
                    if ($atualizarRelatorio_action){
                        echo "<script>finishAction('Atualização Realizada com Sucesso!<br><br>Você será redirecionado em  ', 'relatorios.php')</script>";
                    } else {
                ...
                $query_atualizarAluno = "UPDATE alunos SET nome = \"$nome\", cpf = \"$cpf\", telefone = \"$telefone\", email = \"$email\", vinculo = \"$vinculo\", usuario_ativo = \"$status\", empresa_id = \"$empresa_id\", escola_id = \"$escola_id\", cep = \"$cep\", logradouro = \"$logradouro\", num = \"$num\", bairro = \"$bairro\", complemento = \"$complemento\", cidade = \"$cidade\", estado = \"$estado\" WHERE id = '$id'";
                $atualizarAluno_action = mysqli_query($mysql, $query_atualizarAluno);

                if($atualizarAluno_action){
                    $query_updateUser = "UPDATE usuarios_alunos SET login = \"$login\", nome = \"$nome\" WHERE aluno_id = '$id'";
                    $updateUser_action = mysqli_query($mysql, $query_updateUser);

                    if($updateUser_action){
                        echo "<script>finishAction('Atualização Realizada com Sucesso!<br><br>Você será redirecionado em ', 'usuario.php')</script>";
                    } else {
                        echo "<script>finishAction('Não foi possível realizar a atualização!<br><br>Você será redirecionado em ', 'usuario.php')</script>"; 
                    }
                } else {
                    echo "<script>finishAction('Não foi possível realizar a atualização!<br><br>Você será redirecionado em ', 'usuario.php')</script>";
                }
